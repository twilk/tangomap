// Bakes a <script src="/auth-ui.js" defer> tag into public/tangomap.html's
// __bundler/template <head>, so the auth sign-in/out control loads inside the map bundle.
//
// GUARDED INTEGRATION STEP — this does NOTHING unless invoked explicitly with --write:
//     node scripts/inject-auth-ui.mjs --write
...
export function buildInjected(bundleHtml) {
  ...
  if (i < 0) throw new Error('inject-auth-ui: __bundler/template not found in output');
  const start = i + OPEN.length;
  const end = out.indexOf('</script>', start);
  if (end < 0) throw new Error('inject-auth-ui: template close not found in output');
  const raw = out.slice(start, end).trim();

  // The payload must contain no literal </script (would close the outer bundler tag early)...
  if (/<\/script/i.test(raw)) throw new Error('inject-auth-ui: literal </script inside template payload');
  // ...must still be valid JSON...
  const decoded = JSON.parse(raw);
  // ...must now reference /auth-ui.js...
  if (!decoded.includes(SCRIPT_SRC)) throw new Error('inject-auth-ui: /auth-ui.js missing from decoded template');
  // ...and must not have clobbered existing bundle content.
  if (!decoded.includes('<helmet>')) throw new Error('inject-auth-ui: <helmet> missing from decoded template');
...
if (invokedDirectly && process.argv.includes('--write')) {
  ...
  console.log('inject-auth-ui: dry run (no changes). Pass --write to bake /auth-ui.js into public/tangomap.html.');
}
